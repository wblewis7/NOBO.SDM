...
The detection radius for BBS surveys is 400 m, meaning that expected abundance on BBS surveys is 2% of the abundance within the 5-km x 5-km grid cell. Detections on BBS surveys are modeled based on this survey-level abundance and BBS detection probability, which is modeled based on background noise and the number of passing cars.
...
### fire
Vector giving the standardized valuesfor the number of recorded fires over a 16-year period (fire frequency) in each grid cell in nTot. Values generated from the Monitoring Trends in Burn Severity Burned Areas Boundaries Dataset.
### tmax
...
### snowdays
Vector giving the standardized values of mean number of days with snow depth greater than 2.5 cm in each grid cell in nTot. Values generated from NASA's Daymet v4 dataset.
### pasture
...
### bbs_prop_area
Proportion of 5-km x 5-km grid cell surveyed by each BBS survey (400 m radius circle)
### grid_bbs_id
